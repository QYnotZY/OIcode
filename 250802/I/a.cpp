...
namespace smtb {
    ...
    void update_sqrt(int ql, int qr, int p = 1, int l = 1, int r = n) {
        if(ql <= l && r <= qr) {
            if(tr[p].maxx - tr[p].minn != int(sqrt(tr[p].maxx)) - int(sqrt(tr[p].minn))) {
                int mid = l + r >> 1;
                pushdown(p);
                update_sqrt(ql, qr, p << 1, l, mid);
                update_sqrt(ql, qr, p << 1 | 1, mid + 1, r);
                pushup(p);
                return;
            }
            ll v = int(sqrt(tr[p].minn)) - tr[p].minn;
            upd1(p, v);
            return;
        }
        int mid = l + r >> 1;
        pushdown(p);
        if(ql <= mid) update_sqrt(ql, qr, p << 1, l, mid);
        if(qr > mid) update_sqrt(ql, qr, p << 1 | 1, mid + 1, r);
        pushup(p);
    }
