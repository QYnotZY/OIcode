...
namespace work {
const int mod = 998244353; const int ub = 5e5, N = ub + 10;
...
inline int A(int n, int m) {return n < m ? 0 : Mult(fac[n], ifac[n - m]);}
inline int C(int n, int m) {return n < m ? 0 : Mult(fac[n], Mult(ifac[n - m], ifac[m]));}
...
void Hutao() {
    ...
    for(int i = 0; i <= n && i <= m; i ++)
        ans = Add(ans, Mult(Pow_1(i), Mult(Mult(C(m, i), A(n, i)), Mult(Pow(m + 1, n - i), Pow(n + 1, m - i)))));
    printf("%d", ans);
}
}
int main() {
