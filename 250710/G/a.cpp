...
void Hutao() {
    ...
    s = (r + 1) * c * 2 + 1, t = (r + 1) * c * 2 + 2;
    for(int i = 1; i <= r; i ++) {
        for(int j = 1; j <= c; j ++) {
            int h = readch() - '0';
            adde(i * c + j, i * c + j + r * c, h);
        }
    }
    for(int i = 1; i <= r; i ++) {
        for(int j = 1; j <= c; j ++) {
            char ch = readch();
            if(ch == 'L') adde(s, i * c + j, 1), ans ++;
            if(i <= d || i >= r - d + 1 || j <= d || j >= c - d + 1) adde(i * c + j + r * c, t, inf);
        }
    }
    for(int i = 1; i <= r; i ++) {
        for(int j = 1; j <= c; j ++) {
            for(int i1 = 1; i1 <= r; i1 ++) {
                for(int j1 = 1; j1 <= c; j1 ++) {
                    if((i - i1) * (i - i1) + (j - j1) * (j - j1) <= d * d && (i != i1 || j != j1)) {
                        adde(i * c + j + r * c, i1 * c + j1, inf);
                        adde(i1 * c + j1 + r * c, i * c + j, inf);
                    }
                }
            }
        }
