...
void tarjan(int u) {
    dfn[u] = low[u] = ++ cur, stk[++ top] = u, instk[u] = 1;
    for(auto v : to1[u]) {
        if(!dfn[v]) {
            tarjan(v);
...
void Hutao() {
    n = read(), d = read();
    s = n * (d + 1) + 1, t = n * (d + 1) + 2;
    for(int i = 1; i <= n * (d + 1) + 2; i ++) vector<node> ().swap(e[i]);
    for(int i = 1; i <= n; i ++) {
        vector<int> ().swap(to1[i]);
        dfn[i] = low[i] = cur = scc[i] = cntscc = 0;
    }
    for(int i = 1; i <= d * n; i ++) {
        a[i] = read(), b[i] = read();
        adde(s, n + i, 1);
    ...
    for(int i = 1; i <= n; i ++) adde(i, t, d);
    if(maxflow::Dinic() < d * n) {
        printf("No\n");
        return;
    }
    for(int i = 1; i <= n; i ++) {
        for(auto ele : e[i]) {
            ...
            if(ele.lim == 1) {
                if(a[ele.to - n] == i)
                    to1[i].emplace_back(b[ele.to - n]);
                else
                    to1[i].emplace_back(a[ele.to - n]);
            }
        }
    }
